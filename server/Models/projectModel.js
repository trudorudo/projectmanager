...
const {projectListFormatter} = require('../formatters')
...
const deleteProject = async (id) => {
  return {data: await connect.query(sqlDeleteProject, [id]), status: 200}
}
...
const updateProject = async (id, name, code) => {
  ...
  return {data: await connect.query(sqlUpdateProject, [id, name, code]), status: 200}
}
...
const createNewProject = async (name, code) => {
  ...
  return {data: await connect.query(sqlInsertProject, [name, code]), status: 200}
}
...
const getAllProjects = async (limit = 99999, offset = 0) => {
  ...
  return {data : projectListFormatter(await connect.query(sqlProjects, [limit, offset])), status: 200}
}

module.exports = {deleteProject, updateProject, createNewProject, getAllProjects}
